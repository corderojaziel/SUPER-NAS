...
UNDERSCORED_RE = re.compile(
    r"^([0-9a-f-]+)_([0-9a-f]{2})_([0-9a-f]{2})_([0-9a-f-]+)\.mp4$",
    re.IGNORECASE,
)
FLAT_RE = re.compile(r"^([0-9a-f-]+)\.mp4$", re.IGNORECASE)
...
def canonical_rel_from_legacy(path: Path, upload_root: Path, index: dict[str, Path]) -> str | None:
    name = path.name
    match = UNDERSCORED_RE.match(name)
    if match:
...
def main() -> int:
    ...
    for src in sorted(legacy_root.glob("*.mp4")):
