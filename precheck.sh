...
for f in maintenance/mount-guard.sh maintenance/retry-quarantine.sh maintenance/night-run.sh maintenance/video-optimize.sh maintenance/backup.sh maintenance/smart-check.sh maintenance/post-upload-check.sh maintenance/temp-clean.sh maintenance/zram-nas-apply.sh scripts/nas-alert.sh verify.sh; do [ -f "$SCRIPT_DIR/$f" ] && ok "Archivo presente: $f" || fail "Archivo faltante: $f"; done
