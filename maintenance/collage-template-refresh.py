...
def normalize_decor_family(value: object, fallback: str="editorial") -> str:
    raw = str(value or "").strip().lower()
...
def template_signature(template: dict) -> str:
    ...
    return "|".join([
        ...
        str(template.get("visual_system", "") or template.get("decor_family", "")),
...
def validate_template(template: object, index: int, today: str, season: str) -> Tuple[Optional[dict], str]:
    ...
    normalized = {
        ...
        "visual_system": normalize_decor_family(
            template.get("visual_system") or template.get("decor_family") or template.get("style_family") or "editorial"
        ),
        "decor_family": normalize_decor_family(
            template.get("visual_system") or template.get("decor_family") or template.get("style_family") or "editorial"
        ),
...
Quiero variedad de lenguaje visual, no solo de geometria. Mezcla sistemas como:
editorial, postcard, ticket, scrapbook, notebook, route, travel, confetti, ribbon,
minimal y floral.
...
- Cambia tambien visual_system; no hagas toda la tanda floral ni todo con el mismo lenguaje.
